...
from ResidualAttentionNet import Attention50
...
test_dataloader = DataLoader(OUISIRgait(test_keys, data2), shuffle=False, batch_size=BATCH_SIZE, drop_last=True)
...
class GaitMix(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, num_layers, att_size):
        ...
        self.linear_img1 = nn.Linear(2048, 1024)
        self.linear_img2 = nn.Linear(1024, self.multi_model_size)
    ...
    def forward(self, pose_vec, img, gait_feat):
        ...
        Concat_shared = torch.cat((shared_pose, resnet_output.view(N, 8, self.resnet_final_size)), dim = 1)
        ...
        # silhouette module
        img1 = D(R(self.linear_img2(D(R(self.linear_img1(resnet_output))))))
        
        del P_encoder, hidden, G_encoder
...
for e in range(epochs):
    ...
    for i, data in enumerate(train_dataloader):
        ...
        k = [a.cpu().numpy() for i in range(32)]
        
        y = []
        for i in age_label.float().cpu().numpy():
            temp = [i for j in range(86)]
            y.append(temp)
            
        y = np.array(y)
        
        gp = gaussian(k, y, sigma=1.0)
        
        age_pred = (output_softmax * a).sum(1, keepdim=True).cpu().data.numpy()

        kl_loss = criterion1(output_softmax.log(), torch.Tensor(gp).to(device=device))
        loss_l1 = criterion2(torch.Tensor(age_pred).to(device=device), age_label.unsqueeze(1).float())
        
        loss_2 = kl_loss + loss_l1
    ...
    print('----------------')
    if os.path.isfile('./Results/result.txt'):
        with open('./Results/240304_all_result.txt', 'a') as f:
            f.write("Epoch: {}, Training Gender Accuracy: {}, Age MAE: {}".format(e+1 ,accuracy, mae))
    else:
    ...
    with torch.no_grad():
        ...
        testing_gender_loss = 0
        testing_age_loss = 0
       
        testing_gender_correct = 0
        testing_age_predicted = list()
        testing_age_groudtruth = list()
        ...
        for i, data in enumerate(test_dataloader):
            ...
            testing_gender_loss += loss_1.item()
            testing_age_loss += loss_2.item()
            ...
            for i in range(len(gender_label)):
                if gender_predicted[i] == gender_label[i]:
                    testing_gender_correct += 1
            
            testing_age_predicted.extend(np.ravel(age_pred, order='C'))
            testing_age_groudtruth.extend(age_label.tolist())
            
        avgTestGenderLoss = testing_gender_loss/len(test_dataloader.dataset)
        avgTestAgeLoss = testing_age_loss/len(test_dataloader.dataset)    
        
        ## age MAE
        mae = mean_absolute_error(testing_age_groudtruth, testing_age_predicted)
        ...
        for n in range(len(testing_age_predicted)):
            if abs(testing_age_groudtruth[n]-testing_age_predicted[n]) <= 1:
                cs1_sum += 1
                cs5_sum += 1
                cs10_sum += 1
            elif abs(testing_age_groudtruth[n]-testing_age_predicted[n]) <= 5:
                cs5_sum += 1
                cs10_sum += 1
            elif abs(testing_age_groudtruth[n]-testing_age_predicted[n]) <= 10:
                cs10_sum += 1

        cs1 = cs1_sum/len(testing_age_predicted)
        cs5 = cs5_sum/len(testing_age_predicted)
        cs10 = cs10_sum/len(testing_age_predicted)
        
        
        ## gender accuracy            
        accuracy = testing_gender_correct/len(testing_age_predicted)
        
        print("Testing Losses: Gender: {}, Age: {}".format(avgTestGenderLoss, avgTestAgeLoss))
        print("Testing Gender Accuracy: {}, Age MAE: {}, CS(1): {}, CS(5): {}, CS(10): {}".format(accuracy, mae, cs1, cs5, cs10))
        print('----------------')
        
    avgTrainLoss = total_training_loss / train_steps
    avgTestLoss = total_testing_loss / test_steps
    
    print('Average Losses — Training: {} | Testing: {}'.format(avgTrainLoss, avgTestLoss))
    print()
    ...
    logger["test_loss"].append(avgTestLoss.cpu().detach().numpy())
    logger["test_gender_loss"].append(avgTestGenderLoss)
    logger["test_age_loss"].append(avgTestAgeLoss)
    ...
    if os.path.isfile('./Results/result.txt'):
        with open('../Results/result.txt', 'a') as f:
            f.write(", Testing Gender Accuracy: {}, Age MAE: {}, CS(1): {}, CS(5): {}, CS(10): {}".format(accuracy, mae, cs1, cs5, cs10) + '\n')
    else:
        with open('./Results/result.txt', 'w') as f:
            f.write(", Testing Gender Accuracy: {}, Age MAE: {}, CS(1): {}, CS(5): {}, CS(10): {}".format(accuracy, mae, cs1, cs5, cs10) + '\n')

print('Training & Testing compelete!')